...
toggle-mute() {
  ...
  notify-send -e "$title: $state" -a string:volume -h string:x-canonical-private-synchronous:volume -i "$icon" -r $ID -u low
}
...
set-volume() {
  ...
  notify-send -e -u low "$title: ${new_vol}%" -a string:volume -h string:x-canonical-private-synchronous:volume -h int:value:$new_vol -i "$icon" -r $ID
}
